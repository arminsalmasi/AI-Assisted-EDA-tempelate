...
def run_descriptive_analysis():
    ...
    order_numeric_cols = ['items_count', 'order_value_usd', 'discount_amount_usd', 'net_value_usd', 'delay_minutes', 'customer_rating']
    ...
    # Preferred Device
    device_dist = customers['preferred_device'].value_counts()
    device_pct = customers['preferred_device'].value_counts(normalize=True) * 100
    device_summary = pd.DataFrame({'Count': device_dist, 'Percentage (%)': device_pct})
    
    # Customer Segment
    segment_dist = customers['customer_segment'].value_counts()
    segment_pct = customers['customer_segment'].value_counts(normalize=True) * 100
    ...
    vendor_dist = orders['vendor_category'].value_counts()
    vendor_pct = orders['vendor_category'].value_counts(normalize=True) * 100
    vendor_summary = pd.DataFrame({'Count': vendor_dist, 'Percentage (%)': vendor_pct})
    
    # Delivery Type (from orders)
    delivery_dist = orders['delivery_type'].value_counts()
    delivery_pct = orders['delivery_type'].value_counts(normalize=True) * 100
    delivery_summary = pd.DataFrame({'Count': delivery_dist, 'Percentage (%)': delivery_pct})
    
    # State distribution (from customers)
    state_dist = customers['state'].value_counts()
    state_pct = customers['state'].value_counts(normalize=True) * 100
    ...
    with open(report_path, 'w') as f:
        ...
        f.write("Summary statistics based on all 73,022 transactions in the dataset:\n\n")
        f.write(order_stats.to_markdown() + "\n\n")
        
        # Section 2
        f.write("## 2. Customer-Level Aggregated Statistics\n")
        f.write("Summary statistics based on 5,000 customers aggregated from signup date to latest transaction:\n\n")
