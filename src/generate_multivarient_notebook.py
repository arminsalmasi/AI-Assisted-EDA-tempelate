...
segment_promo_churn = customers.groupby(['customer_segment', 'promo_tier'], observed=False)['is_churned'].mean() * 100
...
3. **Conditional Promotion Effects across Segments**:
   - For **Premium** customers, higher promotion usage is associated with *flat or slightly lower* churn rates.
   - For **Regular** customers, extreme promotion usage (>80% promo ratio) is linked with a *higher* churn rate (nearly 60%). This suggests that heavy promo reliance in regular segments may act as a signal of cherry-picking behavior, where users churn once incentives are removed.
"""))
