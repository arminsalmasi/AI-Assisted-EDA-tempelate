...
def load_first_order_rows():
    ...
    cols_to_use = [
        'user_id', 'gross_amount_usd', 'quantity_items', 'late_minutes',
        'user_rating', 'late_status', 'accuracy_status', 'coupon_applied',
        'coupon_discount_usd', 'merchant_category', 'fulfillment_type'
    ]
    
    merged = pd.merge(
        customers[[
            'user_id', 'account_segment', 'device_type',
...
def build_report():
    ...
    windows = [30, 60, 90]
    lines = [
        "FlashDelivery Hypothesis Testing Results",
        "",
        "Method note",
    ...
    for w in windows:
        ...
        # late_status vs retention
        for val in sorted(cohort['late_status'].unique()):
            subset = cohort[cohort['late_status'] == val]
            retained = subset[outcome_col].sum()
            lines.append(f"- late_status = {val}: {format_rate(retained, len(subset))}")
            
        # accuracy_status vs retention
        for val in sorted(cohort['accuracy_status'].unique()):
            subset = cohort[cohort['accuracy_status'] == val]
            retained = subset[outcome_col].sum()
            lines.append(f"- accuracy_status = {val}: {format_rate(retained, len(subset))}")
            
        # late_minutes by outcome
        ret_delays = cohort[cohort[outcome_col] == 1]['late_minutes']
        not_delays = cohort[cohort[outcome_col] == 0]['late_minutes']
        ...
        # user_rating by outcome
        ret_ratings = cohort[cohort[outcome_col] == 1]['user_rating']
        not_ratings = cohort[cohort[outcome_col] == 0]['user_rating']
        ...
        # Statistical tests
        # Chi-square late status
        contingency_status = pd.crosstab(cohort['late_status'], cohort[outcome_col])
        chi2_status, p_status, dof_status, _ = stats.chi2_contingency(contingency_status)
        
        # Chi-square accuracy status
        contingency_accuracy = pd.crosstab(cohort['accuracy_status'], cohort[outcome_col])
        ...
        formula = f"{outcome_col} ~ late_minutes + user_rating + C(accuracy_status, Treatment('Accurate')) + I(gross_amount_usd / 10.0) + quantity_items + C(account_segment, Treatment('Intro')) + C(device_type, Treatment('App'))"
        ...
        if isinstance(model, str):
            ...
            # late_minutes
            coef = model.params['late_minutes']
            se = model.bse['late_minutes']
            pval = model.pvalues['late_minutes']
            lines.append(f"- late_minutes: OR={np.exp(coef):.3f}, 95% CI [{np.exp(coef - 1.96*se):.3f}, {np.exp(coef + 1.96*se):.3f}], p={format_pvalue(pval)}")
            # user_rating
            coef = model.params['user_rating']
            se = model.bse['user_rating']
            pval = model.pvalues['user_rating']
            lines.append(f"- user_rating: OR={np.exp(coef):.3f}, 95% CI [{np.exp(coef - 1.96*se):.3f}, {np.exp(coef + 1.96*se):.3f}], p={format_pvalue(pval)}")
            # accuracy_incomplete
            coef = model.params["C(accuracy_status, Treatment('Accurate'))[T.Incomplete]"]
            se = model.bse["C(accuracy_status, Treatment('Accurate'))[T.Incomplete]"]
            pval = model.pvalues["C(accuracy_status, Treatment('Accurate'))[T.Incomplete]"]
            lines.append(f"- accuracy_incomplete: OR={np.exp(coef):.3f}, 95% CI [{np.exp(coef - 1.96*se):.3f}, {np.exp(coef + 1.96*se):.3f}], p={format_pvalue(pval)}")
            # accuracy_incorrect
            coef = model.params["C(accuracy_status, Treatment('Accurate'))[T.Incorrect]"]
            se = model.bse["C(accuracy_status, Treatment('Accurate'))[T.Incorrect]"]
            pval = model.pvalues["C(accuracy_status, Treatment('Accurate'))[T.Incorrect]"]
            lines.append(f"- accuracy_incorrect: OR={np.exp(coef):.3f}, 95% CI [{np.exp(coef - 1.96*se):.3f}, {np.exp(coef + 1.96*se):.3f}], p={format_pvalue(pval)}")
    ...
    for w in windows:
        ...
        cohort = df[(df[obs_flag_col] == 1) & (df[outcome_col].notna())].copy()
        cohort[outcome_col] = cohort[outcome_col].astype(int)
        promo_users = cohort[cohort['coupon_applied'] == 'Yes'].copy()
        ...
        # coupon_applied vs retention
        for val in sorted(cohort['coupon_applied'].unique()):
            subset = cohort[cohort['coupon_applied'] == val]
            retained = subset[outcome_col].sum()
            lines.append(f"- coupon_applied = {val}: {format_rate(retained, len(subset))}")
            
        # coupon_discount_usd overall
        ret_disc = cohort[cohort[outcome_col] == 1]['coupon_discount_usd']
        not_disc = cohort[cohort[outcome_col] == 0]['coupon_discount_usd']
        ...
        # coupon_discount_usd promo users only
        if len(promo_users) > 0:
            ret_disc_promo = promo_users[promo_users[outcome_col] == 1]['coupon_discount_usd']
            not_disc_promo = promo_users[promo_users[outcome_col] == 0]['coupon_discount_usd']
        ...
        # Statistical tests
        # Chi-square promo
        contingency_promo = pd.crosstab(cohort['coupon_applied'], cohort[outcome_col])
        ...
        lines.append("Adjusted logistic regression")
        formula_h2 = f"{outcome_col} ~ C(coupon_applied, Treatment('No')) + I(gross_amount_usd / 10.0) + quantity_items + late_minutes + user_rating + C(account_segment, Treatment('Intro')) + C(device_type, Treatment('App')) + C(fulfillment_type, Treatment('Instant')) + C(merchant_category, Treatment('Eatery'))"
        ...
        if isinstance(model_h2, str):
            ...
            coef = model_h2.params["C(coupon_applied, Treatment('No'))[T.Yes]"]
            se = model_h2.bse["C(coupon_applied, Treatment('No'))[T.Yes]"]
            pval = model_h2.pvalues["C(coupon_applied, Treatment('No'))[T.Yes]"]
        ...
        if len(promo_users) > 0:
            ...
            formula_depth = f"{outcome_col} ~ coupon_discount_usd + I(gross_amount_usd / 10.0) + quantity_items + late_minutes + user_rating + C(account_segment, Treatment('Intro')) + C(device_type, Treatment('App'))"
            ...
            if isinstance(model_depth, str):
                ...
                coef = model_depth.params['coupon_discount_usd']
                se = model_depth.bse['coupon_discount_usd']
                pval = model_depth.pvalues['coupon_discount_usd']
    ...
    lines.extend([
        "Overall conclusion",
        "- Hypothesis 1 is supported in direction, with the strongest and most consistent signal coming from user rating. Customers with delayed first orders also show lower retention descriptively, but delay minutes are not a stable adjusted predictor once the other covariates are included.",
